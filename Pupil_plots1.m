         ...
         if ~isempty(sub(s).ex(e).d)
            ...
            if sub(s).con == 0
               ...
               for d = 1:3
                  ...
                  en(e).d(d).pa(en(e).i,:) = pix2mm2(nanmean(sub(s).ex(e).d(d).pa_m));
            ...
            elseif sub(s).con == 1
               ...
               for d = 1:3
                  ...
                  ey(e).d(d).pa(ey(e).i,:) = pix2mm2(nanmean(sub(s).ex(e).d(d).pa_m) );
...
title(  'Time Point 3 (6-12 months)'        );
set(gca, 'xticklabel', {}                          );
elseif d == 2 
...
nlp_axes_prep2(h_axes1,2);
hold on
set(gca, 'Position',   [0.75 5.8 5.85 3.8]               );     %  Left  Btm  Width Height                  
set(gca, 'FontName',   'Arial'                          );
set(gca, 'FontSize',   12                               );
set(gca, 'FontWeight', 'Normal'                         );
set(gca, 'ylim',       [0 6]                           );
...
set(gca, 'xticklabel', {'TP1','TP2','TP3','TP4'} );
set(gca, 'TickLabelInterpreter', 'none'                 );
set(gca, 'clipping',  'off'                             );
set(gca, 'LineWidth', 2                                 );
...
nlp_axes_prep2(h_axes1,2);
hold on
set(gca, 'Position',   [0.75 0.9 5.8 3.8]              );     %  Left  Btm  Width Height                  
set(gca, 'FontName',   'Arial'                          );
set(gca, 'FontSize',   12                               );
set(gca, 'FontWeight', 'Normal'                         );
set(gca, 'ylim',       [-3 0]                        );
...
%Making a table of effect sizes for control TP 1 vs TP2 annd concussed TP1
%and TP2
...
for j = 1:3
    ...
    %Min constrict 
     ...
     effectsize = meanEffectSize(en(1).d(j).min_constrict, en(2).d(j).min_constrict, effect="Cohen");
    d2(i).effectsize{j,1} = num2str(effectsize{1,1}, '%3.2f');
    if effectsize{1,1} < 0
        [auc, p] = rocshuf(en(1).d(j).min_constrict,en(2).d(j).min_constrict,1000);
    else
        [auc, p] = rocshuf(en(2).d(j).min_constrict,en(1).d(j).min_constrict,1000);
    end

    effectsize = meanEffectSize(ey(1).d(j).min_constrict, ey(2).d(j).min_constrict, effect="Cohen");
    d2(i).effectsize{j,2} = num2str(effectsize{1,1}, '%3.2f');
    if effectsize{1,1} < 0
        [auc, p] = rocshuf(ey(1).d(j).min_constrict,ey(2).d(j).min_constrict,1000);
    else
        [auc, p] = rocshuf(ey(2).d(j).min_constrict,ey(1).d(j).min_constrict,1000);
    end

    %Max constrict

    effectsize = meanEffectSize(en(1).d(j).max_constrict, en(2).d(j).max_constrict, effect="Cohen");
    d2(i).effectsize{j,3} = num2str(effectsize{1,1}, '%3.2f');
    if effectsize{1,1} < 0
        [auc, p] = rocshuf(en(1).d(j).max_constrict,en(2).d(j).max_constrict,1000);
    else
        [auc, p] = rocshuf(en(2).d(j).max_constrict,en(1).d(j).max_constrict,1000);
    end

    effectsize = meanEffectSize(ey(1).d(j).max_constrict, ey(2).d(j).max_constrict, effect="Cohen");
    d2(i).effectsize{j,4} = num2str(effectsize{1,1}, '%3.2f');
    if effectsize{1,1} < 0
        [auc, p] = rocshuf(ey(1).d(j).max_constrict,ey(2).d(j).max_constrict,1000);
    else
        [auc, p] = rocshuf(ey(2).d(j).max_constrict,ey(1).d(j).max_constrict,1000);
    end
 
    %T to Max Constrict

    effectsize = meanEffectSize(en(1).d(j).t_max_constrict, en(2).d(j).t_max_constrict, effect="Cohen");
    d2(i).effectsize{j,5} = num2str(effectsize{1,1}, '%3.2f');
    if effectsize{1,1} < 0
        [auc, p] = rocshuf(en(1).d(j).t_max_constrict,en(2).d(j).t_max_constrict,1000);
    else
        [auc, p] = rocshuf(en(2).d(j).t_max_constrict,en(1).d(j).t_max_constrict,1000);
    end

    effectsize = meanEffectSize(ey(1).d(j).t_max_constrict, ey(2).d(j).t_max_constrict, effect="Cohen");
    d2(i).effectsize{j,6} = num2str(effectsize{1,1}, '%3.2f');
    if effectsize{1,1} < 0
        [auc, p] = rocshuf(ey(1).d(j).t_max_constrict,ey(2).d(j).t_max_constrict,1000);
    else
        [auc, p] = rocshuf(ey(2).d(j).t_max_constrict,ey(1).d(j).t_max_constrict,1000);
    end

     %Max COn Vel

    effectsize = meanEffectSize(en(1).d(j).max_con_vel, en(2).d(j).max_con_vel, effect="Cohen");
    d2(i).effectsize{j,7} = num2str(effectsize{1,1}, '%3.2f');
    if effectsize{1,1} < 0
        [auc, p] = rocshuf(en(1).d(j).max_con_vel,en(2).d(j).max_con_vel,1000);
    else
        [auc, p] = rocshuf(en(2).d(j).max_con_vel,en(1).d(j).max_con_vel,1000);
    end

    effectsize = meanEffectSize(ey(1).d(j).max_con_vel, ey(2).d(j).max_con_vel, effect="Cohen");
    d2(i).effectsize{j,8} = num2str(effectsize{1,1}, '%3.2f');
    if effectsize{1,1} < 0
        [auc, p] = rocshuf(ey(1).d(j).max_con_vel,ey(2).d(j).max_con_vel,1000);
    else
        [auc, p] = rocshuf(ey(2).d(j).max_con_vel,ey(1).d(j).max_con_vel,1000);
    end

 %T to Max COn Vel

    effectsize = meanEffectSize(en(1).d(j).t_max_con_vel, en(2).d(j).t_max_con_vel, effect="Cohen");
    d2(i).effectsize{j,9} = num2str(effectsize{1,1}, '%3.2f');
    if effectsize{1,1} < 0
        [auc, p] = rocshuf(en(1).d(j).t_max_con_vel,en(2).d(j).t_max_con_vel,1000);
    else
        [auc, p] = rocshuf(en(2).d(j).t_max_con_vel,en(1).d(j).t_max_con_vel,1000);
    end

    effectsize = meanEffectSize(ey(1).d(j).t_max_con_vel, ey(2).d(j).t_max_con_vel, effect="Cohen");
    d2(i).effectsize{j,10} = num2str(effectsize{1,1}, '%3.2f');
    if effectsize{1,1} < 0
        [auc, p] = rocshuf(ey(1).d(j).t_max_con_vel,ey(2).d(j).t_max_con_vel,1000);
    else
        [auc, p] = rocshuf(ey(2).d(j).t_max_con_vel,ey(1).d(j).t_max_con_vel,1000);
    end


     %Max dil Vel

    effectsize = meanEffectSize(en(1).d(j).max_dil_vel, en(2).d(j).max_dil_vel, effect="Cohen");
    d2(i).effectsize{j,11} = num2str(effectsize{1,1}, '%3.2f');
    if effectsize{1,1} < 0
        [auc, p] = rocshuf(en(1).d(j).max_dil_vel,en(2).d(j).max_dil_vel,1000);
    else
        [auc, p] = rocshuf(en(2).d(j).max_dil_vel,en(1).d(j).max_dil_vel,1000);
    end

    effectsize = meanEffectSize(ey(1).d(j).max_dil_vel, ey(2).d(j).max_dil_vel, effect="Cohen");
    d2(i).effectsize{j,12} = num2str(effectsize{1,1}, '%3.2f');
    if effectsize{1,1} < 0
        [auc, p] = rocshuf(ey(1).d(j).max_dil_vel,ey(2).d(j).max_dil_vel,1000);
    else
        [auc, p] = rocshuf(ey(2).d(j).max_dil_vel,ey(1).d(j).max_dil_vel,1000);
    end

      %T to Max dil Vel

    effectsize = meanEffectSize(en(1).d(j).t_max_dil_vel, en(2).d(j).t_max_dil_vel, effect="Cohen");
    d2(i).effectsize{j,13} = num2str(effectsize{1,1}, '%3.2f');
    if effectsize{1,1} < 0
        [auc, p] = rocshuf(en(1).d(j).t_max_dil_vel,en(2).d(j).t_max_dil_vel,1000);
    else
        [auc, p] = rocshuf(en(2).d(j).t_max_dil_vel,en(1).d(j).t_max_dil_vel,1000);
    end

    effectsize = meanEffectSize(ey(1).d(j).t_max_dil_vel, ey(2).d(j).t_max_dil_vel, effect="Cohen");
    d2(i).effectsize{j,14} = num2str(effectsize{1,1}, '%3.2f');
    if effectsize{1,1} < 0
        [auc, p] = rocshuf(ey(1).d(j).t_max_dil_vel,ey(2).d(j).t_max_dil_vel,1000);
    else
        [auc, p] = rocshuf(ey(2).d(j).t_max_dil_vel,ey(1).d(j).t_max_dil_vel,1000);
    end

     %T75

    effectsize = meanEffectSize(en(1).d(j).T75, en(2).d(j).T75, effect="Cohen");
    d2(i).effectsize{j,15} = num2str(effectsize{1,1}, '%3.2f');
    if effectsize{1,1} < 0
        [auc, p] = rocshuf(en(1).d(j).T75,en(2).d(j).T75,1000);
    else
        [auc, p] = rocshuf(en(2).d(j).T75,en(1).d(j).T75,1000);
    end

    effectsize = meanEffectSize(ey(1).d(j).T75, ey(2).d(j).T75, effect="Cohen");
    d2(i).effectsize{j,16} = num2str(effectsize{1,1}, '%3.2f');
    if effectsize{1,1} < 0
        [auc, p] = rocshuf(ey(1).d(j).T75,ey(2).d(j).T75,1000);
    else
        [auc, p] = rocshuf(ey(2).d(j).T75,ey(1).d(j).T75,1000);
    end

    %avg con vel

    effectsize = meanEffectSize(en(1).d(j).avg_con_vel, en(2).d(j).avg_con_vel, effect="Cohen");
    d2(i).effectsize{j,17} = num2str(effectsize{1,1}, '%3.2f');
    if effectsize{1,1} < 0
        [auc, p] = rocshuf(en(1).d(j).avg_con_vel,en(2).d(j).avg_con_vel,1000);
    else
        [auc, p] = rocshuf(en(2).d(j).avg_con_vel,en(1).d(j).avg_con_vel,1000);
    end

    effectsize = meanEffectSize(ey(1).d(j).avg_con_vel, ey(2).d(j).avg_con_vel, effect="Cohen");
    d2(i).effectsize{j,18} = num2str(effectsize{1,1}, '%3.2f');
    if effectsize{1,1} < 0
        [auc, p] = rocshuf(ey(1).d(j).avg_con_vel,ey(2).d(j).avg_con_vel,1000);
    else
        [auc, p] = rocshuf(ey(2).d(j).avg_con_vel,ey(1).d(j).avg_con_vel,1000);
    end

     %avg dil vel

    effectsize = meanEffectSize(en(1).d(j).avg_dil_vel, en(2).d(j).avg_dil_vel, effect="Cohen");
    d2(i).effectsize{j,19} = num2str(effectsize{1,1}, '%3.2f');
    if effectsize{1,1} < 0
        [auc, p] = rocshuf(en(1).d(j).avg_dil_vel,en(2).d(j).avg_dil_vel,1000);
    else
        [auc, p] = rocshuf(en(2).d(j).avg_dil_vel,en(1).d(j).avg_dil_vel,1000);
    end

    effectsize = meanEffectSize(ey(1).d(j).avg_dil_vel, ey(2).d(j).avg_dil_vel, effect="Cohen");
    d2(i).effectsize{j,20} = num2str(effectsize{1,1}, '%3.2f');
    if effectsize{1,1} < 0
        [auc, p] = rocshuf(ey(1).d(j).avg_dil_vel,ey(2).d(j).avg_dil_vel,1000);
    else
        [auc, p] = rocshuf(ey(2).d(j).avg_dil_vel,ey(1).d(j).avg_dil_vel,1000);
    end
